	...
	<form action="/<?= $SCRIPT_NAME ?>" method="get" class="mx-2" id="userform">
		...
		<?php if (existing_installation()) { ?>
			<div class="form-check">
				<input type="checkbox" class="form-check-input" id="check-overwrite" name="overwrite" value="true" disabled checked>
				<label class="form-check-label" for="check-overwrite">升级前删除原来的静态文件目录</label>
			</div>
		<?php } ?>
		<br />
	</form>
...
function download_dzq_main()
{
	...
	$v_version = get_version_from_versioned_zip($version);
	

	
	//如果只升级到2.X，则按之前的处理方式
	if(	version_compare($v_version, '3.0', "<")){
		...
		remove_dir(dirname(__DIR__).'/public_1');
		remove_dir(dirname(__DIR__).'/public_2');
	}else{
...
function upgrade_existing_installation()
{
	...
	foreach ($versions_to_apply as $ver => $actions) {
        ...
        if(!version_compare($ver, $old_version,'==')) {
            $logs .= '升级到 ' . $ver . "\n";
            $logs .= apply_version_upgrade($actions, $ver);
        }
	}
...
function apply_version_upgrade($actions, $ver)
{
	if (count($actions) === 0) return '';
	$logs = '    ';
	foreach ($actions as $type => $params) {
		if(in_array($type, ['command', 'config'])){
			$logs .= call_user_func('apply_version_upgrade_' . $type, $params, $ver);
		}else{
...
function apply_version_upgrade_config($params, $ver)
{
	$logs = "升级config.php文件...";
	$config_file = realpath(__DIR__ . "/../config/config.php");
	$config_str = file_get_contents($config_file);
	$executed_commands_filename = 'apply_version_upgrade_command'.$ver.'.txt';
	if(!file_exists($executed_commands_filename)){
...
function apply_version_upgrade_command($params, $ver)
{
	$logs = "";
	$logs .= run_db_migration();
	//读取是否已执行完成的command 命令
	$executed_commands_filename_old = 'apply_version_upgrade_command'.$ver.'.txt';
	$executed_commands_filename = '../storage/logs/apply_version_upgrade_command'.$ver.'.txt';
	if(file_exists($executed_commands_filename_old)){
		$c_file = fopen($executed_commands_filename_old, 'a');
		$executed_commands_filename = $executed_commands_filename_old;
	}else{
		$c_file = fopen($executed_commands_filename, 'a');
	}

	$executed_commands = file_get_contents($executed_commands_filename);
	foreach ($params as $param) {
		if(strpos($executed_commands, $param) !== false)		continue;
		$logs .= "执行升级命令 ${param}\n";
		$logs .= run_custom_command($param);
		file_put_contents($executed_commands_filename, $param."\n", FILE_APPEND);
	}
	file_put_contents($executed_commands_filename, "执行升级command完成\n", FILE_APPEND);
	fclose($c_file);
//	unlink($executed_commands_filename);		//命令执行完就删掉文件
	return $logs;
}
